...
# -*- coding:utf-8 -*-
# @Author  : Galonestar
# @file       :  ImageGeometryDetection.py
...
class ImageGeometryDetection:
    ...
    def continue_weight(self, centre_point, error=30):  # 连续性权重
        if self.Track == 0:
            ...
            if self.Track == 1:
                ...
                # print('distance', dist)
            elif self.sub_optimal_node is not None and self.Track == 2:
    ...
    def kalman_filter(self, points, prediction=3):
        ...
        self.hit_coordinate = \
        (np.mat([[1, 0, prediction, 0], [0, 1, 0, prediction], [0, 0, 0, 0], [0, 0, 0, 0]]) * x_mat).tolist()[
            0]  # 预测n帧后的坐标
    ...
    def run(self):
        ...
        if self.optimal_node is not None:  # 如果这帧图像拿到了最优节点
            if self.distance(self.optimal_node.point, self.optimal_node.match_node.point) != 0:
